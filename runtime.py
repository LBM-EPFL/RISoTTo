...
def max_pred_to_seq(p: pt.Tensor):
    return "".join(
        [sp.res3to1[r] for r in sp.std_rna[pt.argmax(p, dim=1).cpu().numpy()]]
    )
...
def superpose(xyz_ref, xyz):
    ...
    # reflection matrix
    Z = pt.zeros(U.shape, device=xyz.device) + pt.eye(
        U.shape[1], U.shape[2], device=xyz.device
    ).unsqueeze(0)
...
def compute_gdt_ts(
    xyz0: pt.Tensor, xyz1: pt.Tensor, r_thr=[1.0, 2.0, 4.0, 8.0]
) -> float:
    # superpose
...
def split_by_residue(structure):
    ...
    residues = [
        sp.atom_select(structure, structure["resid"] == resid) for resid in uresids
    ]
...
def chain_masks(structure):
    ...
    chain_names = [
        structure["chain_name"][i].split(":")[0] for i in pt.max(Mc, dim=0)[1]
    ]
...
def subunit_type(subunit):
    if np.all([rn in sp.resname_to_categ for rn in subunit["resname"]]):
        t = np.unique(
            [
                sp.resname_to_categ[rn]
                for rn in subunit["resname"]
                if rn in sp.resname_to_categ
            ]
        )
        if len(t) == 1:
            return t.item()
...
def self_consistency_secondary(
    ss_ref, ss_pred, keep_pseudoknots: bool = False
) -> float:
    """
...
class Dataset(pt.utils.data.Dataset):
    ...
    def __getitem__(self, k):
        ...
        structure = sp.data_to_structure(
            X.numpy(),
            pt.cat([qe, qr, qn], dim=1).numpy(),
            Mr.numpy(),
            sp.std_elements,
            sp.std_resnames,
            sp.std_names,
...
class ConfidenceMap:
    ...
    def __call__(self, p):
        ...
        return np.clip(
            np.stack(
                [np.interp(p[:, k], self.x, self.C[k]) for k in range(p.shape[1])],
                axis=1,
            ),
            0.0,
            1.0,
...
class SequenceModel:
    def __init__(self, save_path, parameters_filename, device=pt.device("cpu")):
        ...
        self.module = load_module(
            os.path.basename(save_path), os.path.join(save_path, "__init__.py")
        )
        ...
        self.model.load_state_dict(
            pt.load(model_filepath, map_location=pt.device("cpu"))
        )
    ...
    def __call__(self, structure, m_known=None, n_skip=1):
        ...
        if m_known is None:
            ...
            if np.any(m_known):
                mr_known = (
                    pt.max(Mr[pt.from_numpy(m_known).to(Mr.device)], dim=0)[0] > 0.0
                )
            else:
                mr_known = pt.zeros(Mr.shape[1]).bool()
        ...
        m = sp.backbone_mask_cg(qr, qn, sp.std_rna)
        ...
        # get sequence
        m_std_aa = pt.from_numpy(
            np.isin(self.module.std_resnames, self.module.std_rna)
        ).to(qr.device)
        ...
        structure = self.module.data_to_structure(
            X.numpy(),
            q.numpy(),
            Mr.numpy(),
            self.module.std_elements,
            self.module.std_resnames,
            self.module.std_names,
        )
